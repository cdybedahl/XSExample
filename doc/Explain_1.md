...
There is one more thing that should be said about this code: pushing the results like this is exactly equivalent to doing `return (17,42,4711);` in Perl. Which is _not_ the same as `my @foo = (17,42,4711);return @foo;`. The difference is in what happens when a call to the functions gets evaluated in a scalar context. There, the literal list version (or the XS version) evaluates to the last value in the list (4711) while the array version evaluates to the length of the array (3). If you're writing the XSUB because you're replacing a Perl function that used the array return variant, and a lot of the calling code assumes that evaluating the function call in a scalar context will result in a count of the things returned, you may get a not so pleasant surprise (he wrote knowingly). Particularly if what you're returning from the XSUB is not even a number. Anyway, what you need to do in that case is to manually check the context and return the length explicitly if it's scalar. The section "Using GIMME_V" in the perlcall documentation page will tell you exactly how to do that.

And at this point you may, if you've been paying attention, be wondering what the frak "make the `SV` mortal" means.
...
And at least when I run it, it works. I don't think there's all that much to say about these functions. They're exactly like ones we've seen before, except they use the normal Perl object calling convention with the pointer to the object as the first argument, and that argument has the `Example` type. If you don't like the separate setter/getter style, it's of course possible to combine them:
...
Perl comes with a lot of documentation. The parts of it that talk about perl's internals and how to interface to them from C are fairly hard to get into, since they suffer from what someone called "Unix-style documentation syndrome". That is, every part assumes you've already read all the other parts. Which is fine for reference, but makes it quite forbidding for the newbie. After reading this document, I hope you have enough of a sense of how the lower levels of perl work that the actual documentation will be less hard to understand. That said, you still probably will have to come back to some documentation over and over again in order to understand all of it. A useful reading order might be something like this:
